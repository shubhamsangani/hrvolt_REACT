...
const validateFileType = (value: FileList) => {
  ...
  if (!["application/x-zip-compressed"].includes(value[0].type)) {
    return "Only .zip is allowed.";
  }
};

const NewBulkResume: React.FC<Props> = ({ className }) => {
  const { t } = useTranslation();
  ...
  const [compareAnalysis, setcompareAnalysis] =
    useState<JobApplicationDetails | null>(null);
  console.log("🚀 ~ compareAnalysis:", compareAnalysis);
  ...
  const handleBulkResumePage = async (formdata: JobApplication) => {
    ...
    try {
      if (userDetail && initialUserDetail && authTokens) {
        ...
        console.log("🚀 ~ handleBulkResumePage ~ data:", data);
          ...
          <>
            ...
            <div className="row g-6 g-xl-9 mb-6 mb-xl-9">
              <table className="table table-striped">
                ...
                <tbody>
                  {compareAnalysis &&
                    compareAnalysis.aiCompPercentageScore.map(
                      (analysis, index) => (
                        <>
                          <tr key={index} className="text-center">
                            <td className="">Candidate</td>
                            <td className="fw-bold fs-5">{analysis.aiCompPercentageScore} %</td>
                            <td>
                              <div>
                                <a
                                  href="#"
